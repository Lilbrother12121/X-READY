...
let id = messge.content.split(' ').slice(1,2).join(' ');
  let dUser = message.guild.member(message.mentions.users.first()) || message.guild.members.get(id);
  if (!dUser) return message.channel.send("**I Can't Find This User ! You Must Mention Him OR This User Isn't In This Server**")
  ...
  let dMsg = messge.content.split(' ').slice(2).join(' ');
  if(dMsg.length < 1) return message.reply('You must supply a message!')
  
  dUser.send(`${dUser} **This Is Moderator\Administrator\Owner**_${message.author}_**Sent You A Dm From Thier Server  Read It Carefully : ** >> ${dMsg}`)
